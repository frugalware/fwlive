...
extern bool ui_window_locale(const char *var,char **locales,char **locale)
{
  char text[TEXT_MAX] = {0};
  int textbox_width = 0;
  int textbox_height = 0;
  ...
  if(newtCenteredWindow(NEWT_WIDTH,NEWT_HEIGHT,LOCALE_TITLE) != 0)
  {
    eprintf("Failed to open a NEWT window.\n");
    return false;
  }
...
extern bool ui_window_layout(struct layout **layouts,struct layout **layout)
{
  ...
  if(!get_text_screen_size(LAYOUT_TEXT,&textbox_width,&textbox_height))
    return false;
  ...
  if(newtCenteredWindow(NEWT_WIDTH,NEWT_HEIGHT,LAYOUT_TITLE) != 0)
  {
    eprintf("Failed to open a NEWT window.\n");
    return false;
  }
  ...
  newtTextboxSetText(textbox,LAYOUT_TEXT);
  ...
  for( p = layouts ; *p != 0 ; ++p )
    newtListboxAppendEntry(listbox,(*p)->kbdlayout,*p);
  ...
  while(true)
  {
    ...
    if(es.reason == NEWT_EXIT_COMPONENT && es.u.co == next)
    {
      *layout = newtListboxGetCurrent(listbox);
      break;
    }
  }
